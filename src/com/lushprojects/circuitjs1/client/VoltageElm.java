...
class VoltageElm extends CircuitElm {
	...
	void setPoints() {
		super.setPoints();
		calcLeads((waveform == WF_VAR) ? 8 : circleSize * 2);
	}
	...
	void draw(Graphics g) {
		...
		/*if (false) {
			...
			//draw a smile face in the circle
			g.setColor(whiteColor);
			g.context.beginPath();
			g.context.arc(xc, yc, circleSize, 0, 2 * Math.PI);
			g.context.fill();
			g.setColor(Color.black);
			g.context.beginPath();
			g.context.arc(xc - 5, yc - 5, 2, 0, 2 * Math.PI);
			g.context.fill();
		} else {
			*/
	...
	void drawWaveform(Graphics g, Point center) {
		...
		case WF_DC: {
			...
			// draw arrow pointing to  terminal of voltage source near the circle
			int arrowSize = 5; // You can adjust this value
			int dx = lead2.x - lead1.x;
			int dy = lead2.y - lead1.y;
			double arrowLength = Math.sqrt(dx * dx + dy * dy);

			double arrowBaseX = arrowSize;
			double arrowTipYPositive = arrowSize;
			double arrowTipYNegative = -arrowSize;

			double sinTheta = dy / arrowLength;
			double cosTheta = dx / arrowLength;

			// Calculate position for the first arrowhead point
			double firstArrowheadX = arrowBaseX * cosTheta - arrowTipYPositive * sinTheta + lead1.x;
			double firstArrowheadY = arrowBaseX * sinTheta + arrowTipYPositive * cosTheta + lead1.y;

			// Calculate position for the second arrowhead point
			double secondArrowheadX = arrowBaseX * cosTheta - arrowTipYNegative * sinTheta + lead1.x;
			double secondArrowheadY = arrowBaseX * sinTheta + arrowTipYNegative * cosTheta + lead1.y;

			// Calculate the offset vector for the arrow position
			Point arrowOffsetVec = new Point(lead1.y - lead2.y, -(lead1.x - lead2.x));
			arrowOffsetVec.x *= 0.75;
			arrowOffsetVec.y *= 0.75;

			g.setLineWidth(1.5);
			// Draw the lines for the arrowhead
			g.drawLine(lead1.x + arrowOffsetVec.x, lead1.y + arrowOffsetVec.y, (int) firstArrowheadX + arrowOffsetVec.x, (int) firstArrowheadY + arrowOffsetVec.y);
			g.drawLine(lead1.x + arrowOffsetVec.x, lead1.y + arrowOffsetVec.y, (int) secondArrowheadX + arrowOffsetVec.x, (int) secondArrowheadY + arrowOffsetVec.y);
			// Draw the line from the voltage source to the arrowhead base
			g.drawLine(lead2.x + arrowOffsetVec.x, lead2.y + arrowOffsetVec.y, lead1.x + arrowOffsetVec.x, lead1.y + arrowOffsetVec.y);
		...
		if (sim.showValuesCheckItem.getState() && waveform != WF_NOISE && waveform != WF_DC) {
